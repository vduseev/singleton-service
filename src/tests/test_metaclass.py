...
def test_basic(clean_sys_modules):
    ...
    assert TestProvider.__provider_initialize__ is not None
...
def test_initialize_not_classmethod(clean_sys_modules):
    with pytest.raises(ProviderDefinitionError, match="must be decorated with @classmethod"):
        class BadInitProvider(BaseProvider):
            def initialize(cls):
                pass
...
def test_init_not_allowed_on_initialize(clean_sys_modules):
